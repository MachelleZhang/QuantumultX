/*
 * @LICENSE: GPL V3.0
 * @Date: 2021-04-06 15:11:33
*/
const $ = new Env('京东极速版开红包');
...
async function speedRedPacket() {
    try {
        ...
        for(let item of wechatCash){
            await cashForWechat(item);
        }
    } catch (e) {
        $.logErr(e);
    }
}
function springRewardReceive() {
    return new Promise(async resolve => {
        $.get(getUrl('spring_reward_receive', `{"inviter":"${inviterStr}","linkId":"${LinkId}"}`), (err, resp, data) => {
            try {
                if (err) {
...
function springRewardList() {
    return new Promise(async resolve => {
        $.get(getUrl('spring_reward_list', `{"pageNum":1,"pageSize":10,"linkId":"${LinkId}","inviter":"${inviterStr}"}`), (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.success) {
                            ...
                            for(let item of rewardsList){
                                if(item.prizeType === 4 && item.state === 0){
                                    wechatCash.push(item);
                                }
                            }
                        } else {
