	...
	for (let cidsKey of cidsList) {
		for (let typeKey of typeList) {
			if (!cidsMap.hasOwnProperty(cidsKey) || !typeMap.hasOwnProperty(typeKey))
				continue console.log(`⏰ 获取 ${cidsKey} ${typeKey} 商品列表`)
			$.totalPages = 1
...
async function filterGoodList() {
	...
	$.goodList = allGoodList.filter(good => {
		...
		if (!good || good.endTime < now + 10 * 60 * 1000 || good.endTime > oneMoreDay || good.jdPrice <
			minPrice) {
			return false
		}
...
function Env(name, opts) {
	...
	return new(class {
		...
		lodash_set(obj, path, value) {
			...
			path = path.toString().match(/[^.[\]]+/g) || [] path.slice(0, -1).reduce((a, c, i) => (Object(a[c]) === a[c] ? a[c] : (a[c] = Math.abs(path[i + 1]) >> 0 === +path[i + 1] ? [] : {})), obj)[path[path.length - 1]] = value
			return obj
		}
		getdata(key) {
